...
## could import part of data
# list_of_dfs = [pd.read_table(v, names = ['ID', 'Date', 'kWh'], skiprows = 6000000, nrows = 1500000, na_values = missing, sep = " ") for v in paths]

## import all data
list_of_dfs = [pd.read_table(v, names = ['ID', 'Date', 'kWh'], na_values = missing, sep = " ") for v in paths]
df_assign = pd.read_csv(root + assign_file, usecols = [0,1,2,3,4], na_values = missing) # keep only first 5 columns
